...
static const ArgDesc argDesc[] = { { "-f", argInt, &firstPage, 0, "first page to convert" },
                                   ...
                                   { "-fontsizemul", argFP, &fontsizemul, 0, "font size multiplier (default 1.0)"},
                                   ...
                                   { "-svg", argFlag, &svg, 0, "output inkscape SVG"},
                                   {} };
  ...
  if (htmlOut->isOk())
  {
    ...
    if ((complexMode || singleHtml) && !xml && !ignore) {
        ...
        for (int pg = firstPage; pg <= lastPage; ++pg) {
            InMemoryFile imf;
            doc->displayPage(splashOut, pg, resolution * scale, resolution * scale, 0, true, false, false);
