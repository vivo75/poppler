...
void HtmlPage::coalesce()
{
  ...
  printf("\n------------------------------------------------------------\n\n");
    ...
    if (!complexMode && !svg) { /* if not in complex mode get rid of duplicate strings */
        HtmlString *str3;
        bool found;
        while (str1) {
...
void HtmlPage::dumpPathsAsSVG(FILE * file) {
  ...
  for ( int i = 0; i < glPaths->getLength(  ); i++ ) {
    HtmlPath *t = ( HtmlPath * ) glPaths->get( i );
    var = t->toString(  );
    fprintf( file, "%s\n", var->getCString(  ) );
    delete var;
  }
...
HtmlOutputDev::HtmlOutputDev(Catalog *catalogA, const char *fileName, const char *title, const char *author, const char *keywords, const char *subject, const char *date, bool rawOrderA, int firstPage, bool outline)
{
    ...
    if (svg) {
      ...
      fputs( "   xmlns:cc=\"http://creativecommons.org/ns#\"\n", page );
      ...
      fputs( "   xmlns:pdftosvg=\"http://f1r.eu/namespaces/pdftosvg\"\n", page );
      fputs( "   width=\"595.27557pt\"\n", page );
      fputs( "   height=\"841.88977pt\"\n", page );
      fputs( "   viewBox=\"0 0 595 841\"\n", page );
      fputs( "   version=\"1.1\"\n", page );
      fputs( "   id=\"svg1619\"\n", page );
      fputs( "   inkscape:version=\"0.92.2 2405546, 2018-03-11\"\n", page );
      fputs( "   sodipodi:docname=\"page_template.svg\">\n", page );
      fputs( "  <defs\n", page );
      fputs( "     id=\"defs1613\" />\n", page );
      ...
      fputs( "     inkscape:zoom=\"0.7\"\n", page );
      fputs( "     inkscape:cx=\"42.17584\"\n", page );
      fputs( "     inkscape:cy=\"539.96057\"\n", page );
      fputs( "     inkscape:document-units=\"mm\"\n", page );
      fputs( "     inkscape:current-layer=\"layer1\"\n", page );
      fputs( "     showgrid=\"false\"\n", page );
      fputs( "     inkscape:window-width=\"2560\"\n", page );
      fputs( "     inkscape:window-height=\"1363\"\n", page );
      fputs( "     inkscape:window-x=\"0\"\n", page );
      fputs( "     inkscape:window-y=\"0\"\n", page );
      fputs( "     inkscape:window-maximized=\"1\"\n", page );
      fputs( "     units=\"pt\" />\n", page );
      fputs( "  <metadata\n", page );
      fputs( "     id=\"metadata1616\">\n", page );
      fputs( "    <rdf:RDF>\n", page );
      fputs( "      <cc:Work\n", page );
      fputs( "         rdf:about=\"\">\n", page );
      fputs( "        <dc:format>image/svg+xml</dc:format>\n", page );
      fputs( "        <dc:type\n", page );
      fputs( "           rdf:resource=\"http://purl.org/dc/dcmitype/StillImage\" />\n", page );
      fputs( "        <dc:title></dc:title>\n", page );
      fputs( "        <dc:creator>\n", page );
      fputs( "          <cc:Agent>\n", page );
      fputs( "            <dc:title>Poppler::pdftohtml</dc:title>\n", page );
      fputs( "          </cc:Agent>\n", page );
      fputs( "        </dc:creator>\n", page );
      fputs( "      </cc:Work>\n", page );
      fputs( "    </rdf:RDF>\n", page );
